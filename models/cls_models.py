...
class Classinfo(models.Model):
    ...
    member = fields.One2many('living.cls_member', 'cls_id', '班级成员')
    manager = fields.One2many('living.cls_manager', 'cls_id', '班委成员')
    cls_nums = fields.Integer('班级人数', compute='_compute_cls_nums')

    # 每个班级有多个动态
    tag_ids = fields.One2many('living.tag', 'cls_id', '班级动态')
    act_ids = fields.One2many('living.cls_act', 'id', '班级活动')
    ...
    def _compute_cls_nums(self):
        for rec in self:
            ...
            print('_count_cls_nums')


# 班级成员表
class ClsMember(models.Model):
    _name = 'living.cls_member'
