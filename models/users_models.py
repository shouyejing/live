...
class UserInfo(models.Model):
    ...
    city = fields.Char('城市')
    job = fields.Char('职业')
    class_id = fields.Many2one('living.cls_info', '班级')
    gold = fields.Integer('金币', default=0)
    prize = fields.Integer('奖品', default=0)
    #每个用户有多个动态
    tag_ids = fields.One2many('living.tag', 'user_id', '动态')


# 教师信息
class Teacher(models.Model):
    _name = 'living.teacher'
    _description = u'教师信息'
    
    # org  = fields.One2many('living.org', 'org_id', '所属机构')
...
class EmailVerifyRecord(models.Model):
    _name = 'living.email_verifyrecord'
